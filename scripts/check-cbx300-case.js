...
check('wires CBX300 cover into existing ProjectStudy router', function () {
  ...
  assert.ok(index.indexOf('cbx300-case.css?v=s43') !== -1, 'index.html missing growth cache-bust');
  assert.ok(mvIndex.indexOf('cbx300-case.js?v=s23') !== -1, 'multiverse missing growth cache-bust');
...
check('light proof: pressure-ladder stamps and job ghosts', function () {
  ...
  assert.ok(pages.indexOf('Finding: Paid late, chased invoices.') !== -1, 'freelancer stamp finding');
  assert.ok(pages.indexOf('Choice: Pay & get paid on phone.') !== -1, 'freelancer stamp choice');
  assert.ok(pages.indexOf('UI: Home = receive + pay.') !== -1, 'freelancer stamp UI');
  assert.ok(pages.indexOf('“When does my money actually land?”') !== -1, 'freelancer need');
  assert.ok(pages.indexOf('Mobile pay-in and pay-out as one job.') !== -1, 'freelancer fact');
  ...
  assert.ok(pages.indexOf('Finding: One balance number lies.') !== -1, 'sole stamp finding');
  assert.ok(pages.indexOf('Choice: Lead with available.') !== -1, 'sole stamp choice');
  assert.ok(pages.indexOf('UI: Available hero; holds sit back.') !== -1, 'sole stamp UI');
  assert.ok(pages.indexOf('“What can I spend today?”') !== -1, 'sole need');
  assert.ok(pages.indexOf('Available leads; pending never wears the crown.') !== -1, 'sole fact');
  ...
  assert.ok(pages.indexOf('Finding: Approvals become a pile.') !== -1, 'mid stamp finding');
  assert.ok(pages.indexOf('Choice: Prepare ∥ approve, split.') !== -1, 'mid stamp choice');
  assert.ok(pages.indexOf('UI: Waiting-on-me door.') !== -1, 'mid stamp UI');
  assert.ok(pages.indexOf('“What needs me before payroll?”') !== -1, 'mid need');
  assert.ok(pages.indexOf('One queue for her decisions — not everyone’s noise.') !== -1, 'mid fact');
  ...
  assert.ok(pages.indexOf('Phone shows pay and cash') === -1, 'old freelancer fact leaked');
  ...
  assert.ok(pages.indexOf('How much can I safely spend?') === -1, 'old sole need leaked');
  assert.ok(pages.indexOf('Available · Add beneficiary') === -1, 'old sole ghost leaked');
  assert.ok(pages.indexOf('Approving is the job') === -1, 'old mid stamp leaked');
  ...
  assert.ok(css.indexOf('opacity: 0.42') !== -1, 'ghost stays a light proof pass');
